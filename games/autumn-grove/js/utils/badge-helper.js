/**
 * Badge Helper for Autumn Grove Game
 *
...
/**
 * Badge Helper wrapper for Autumn Grove
 * Uses the new Wavelength SDK for badge operations
 */
class AutumnBadgeHelper {
  constructor() {
    ...
    waitForSDK(() => {
      this.sdkReady = true;
      console.log('[AutumnBadgeHelper] SDK ready:', {
  ...
  async awardBadge(badgeIdOrParams, badgeImage, metadata = {}) {
    ...
    if (this.awardedBadges.has(badgeId)) {
      console.log('[AutumnBadgeHelper] Badge already awarded:', badgeId);
    ...
    if (window.Wavelength && window.Wavelength.badges) {
      try {
        ...
        console.error('[AutumnBadgeHelper] Failed to award badge:', error);
      ...
      console.warn('[AutumnBadgeHelper] SDK not available, showing local notification only');
      this.showBadgeNotification(badgeId);
  ...
  showBadgeNotification(badgeId) {
    // Autumn Grove badge definitions for display
    const badges = {
      autumn_walker: { name: "Autumn Walker", rarity: "common", emoji: "🍂" },
      leaf_whisperer: { name: "Leaf Whisperer", rarity: "uncommon", emoji: "🍁" },
      forest_guardian: { name: "Forest Guardian", rarity: "rare", emoji: "🌳" },
      autumn_sage: { name: "Autumn Sage", rarity: "epic", emoji: "🧚‍♀️" },
      grove_keeper: { name: "Grove Keeper", rarity: "legendary", emoji: "💚" }
    };

    const badge = badges[badgeId];
    if (!badge) {
      console.warn('[AutumnBadgeHelper] Unknown badge:', badgeId);
      return;
    }
  ...
  updateBadgeCounter() {
    ...
    if (counter) {
      counter.textContent = this.awardedBadges.size;
    }
  }
  ...
  async getBadges() {
    ...
    if (window.Wavelength && window.Wavelength.badges) {
      try {
        ...
        console.error('[AutumnBadgeHelper] Failed to get badges:', error);
        return [];
      }
    }
    ...
    const badges = {
      autumn_walker: { id: 'autumn_walker', name: "Autumn Walker", rarity: "common", emoji: "🍂" },
      leaf_whisperer: { id: 'leaf_whisperer', name: "Leaf Whisperer", rarity: "uncommon", emoji: "🍁" },
      forest_guardian: { id: 'forest_guardian', name: "Forest Guardian", rarity: "rare", emoji: "🌳" },
      autumn_sage: { id: 'autumn_sage', name: "Autumn Sage", rarity: "epic", emoji: "🧚‍♀️" },
      grove_keeper: { id: 'grove_keeper', name: "Grove Keeper", rarity: "legendary", emoji: "💚" }
    };
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = AutumnBadgeHelper;
}
